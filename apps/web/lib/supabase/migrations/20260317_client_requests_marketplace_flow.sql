-- Client request marketplace flow (radius matching + technician geo/hours)
...
begin
  begin
    alter table public.profiles
      add constraint profiles_service_radius_km_chk
...
begin
  begin
    alter table public.client_requests
      add constraint client_requests_radius_km_chk
      check (radius_km >= 1 and radius_km <= 100);
  exception
    when duplicate_object then null;
  end;
end $$;
...
create index if not exists idx_client_requests_status_updated
  on public.client_requests (status, updated_at desc);

create index if not exists idx_client_requests_geo
  on public.client_requests (location_lat, location_lng);
...
create or replace function public.client_requests_touch_updated_at()
returns trigger
language plpgsql
as $$
begin
...
for each row execute function public.client_requests_touch_updated_at();
