...
const isWeakAutosyncToken = (value: string) => {
  const normalized = value.trim().toLowerCase();
  if (normalized.length < MIN_AUTOSYNC_TOKEN_LENGTH) return true;
  ...
  return blockedTokens.has(normalized);
};

const safeTokenCompare = (a: string, b: string) => {
...
export async function POST(request: NextRequest) {
  ...
  if (!incomingToken || !safeTokenCompare(incomingToken, normalizedToken)) {
