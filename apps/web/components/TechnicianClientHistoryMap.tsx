...
const buildClientMarkerHtml = (index: number, selected: boolean) => `
  <div class="ufx-client-map-pin ${selected ? 'is-selected' : ''}">
...
const buildClientTooltipHtml = (point: TechnicianClientHistoryMapPoint) => `
  <div class="ufx-client-map-tooltip-card">
    ...
    <p class="ufx-client-map-tooltip-subtle">${point.movements} mov. &middot; $${point.totalAmount.toLocaleString('es-AR')}</p>
  </div>
`;

export default function TechnicianClientHistoryMap({ points, selectedPointId = '', onSelectPoint, onOpenMap }: Props) {
  ...
  useEffect(() => {
    ...
    points.forEach((point, index) => {
      const marker = L.marker([point.lat, point.lon], {
        icon: L.divIcon({
          html: buildClientMarkerHtml(index, point.id === selectedPointId),
    ...
    if (selectedPoint) {
      map.flyTo([selectedPoint.lat, selectedPoint.lon], Math.max(map.getZoom(), 14), {
        animate: true,
        duration: 0.55,
      });
      return;
  ...
  }, [mapReady, points, selectedPointId]);

  return (
    <div className="relative isolate min-h-[360px] overflow-hidden rounded-[28px] border border-slate-200 bg-[#fffaf4] shadow-[inset_0_1px_0_rgba(255,255,255,0.84)]">
      ...
      <div className="absolute left-4 top-4 z-[3] inline-flex items-center gap-2 rounded-full border border-white/72 bg-white/90 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#2a0338] shadow-[0_16px_36px_-24px_rgba(42,3,56,0.58)] backdrop-blur">
      ...
      <div className="absolute bottom-3 left-3 right-3 z-[3] rounded-[18px] border border-white/72 bg-white/90 px-3 py-2 shadow-[0_16px_34px_-28px_rgba(42,3,56,0.62)] backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-sm font-semibold text-[#180f24]">Clientes por zona</p>
            <p className="mt-0.5 text-xs text-slate-500">
              {points.length} {points.length === 1 ? 'ubicacion confirmada' : 'ubicaciones confirmadas'}
            </p>
          </div>
          <button
            type="button"
