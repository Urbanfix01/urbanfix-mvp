      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View style={styles.logoSection}>
            {hasLogo ? <Image src={logoUrl} style={styles.logo} /> : null}
            <Text style={styles.companyName}>{profile?.business_name || 'SERVICIO TECNICO'}</Text>
        ...
        <View style={styles.table}>
          <View style={[styles.tableRow, styles.tableHeader]}>
            <Text style={styles.tableCol1}>DESCRIPCION</Text>
          ...
          {safeItems.map((item, i) => (
            <View key={i} style={styles.tableRow}>
              <Text style={styles.tableCol1}>{getItemLabel(item)}</Text>
        ...
        <Text style={styles.footer}>
          Documento generado digitalmente por UrbanFix. Este presupuesto tiene una validez de 15 dias a partir de la fecha de emision.
        </Text>
      </Page>
    </Document>
  );
