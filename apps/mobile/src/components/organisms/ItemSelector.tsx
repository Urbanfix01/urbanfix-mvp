      ...
      <TouchableOpacity style={styles.itemRow} onPress={() => handleSelect(item)}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.itemPrice}>
            {item.suggested_price > 0 ? `$${item.suggested_price.toLocaleString('es-AR')}` : 'Precio a definir'}
            {` | ${sourceLabel}`}
            {unit ? ` | ${unit}` : ''}
          </Text>
          {!!technicalSummary && <Text style={styles.itemTechnical}>{technicalSummary}</Text>}
        </View>
...
const styles = StyleSheet.create({
  ...
  itemTechnical: { fontSize: 11, color: '#475569', marginTop: 4, lineHeight: 15 },
