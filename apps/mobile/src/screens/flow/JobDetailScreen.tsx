...
const shareOrCopyLink = async (link: string, options?: { title?: string; message?: string; copiedLabel?: string }) => {
  ...
  const promptLabel = options?.copiedLabel || 'Link listo para copiar';
...
export default function JobDetailScreen() {
  ...
  const [isPreparingFeedbackLink, setIsPreparingFeedbackLink] = useState(false);
  ...
  const handleShare = async () => {
    ...
    const quoteReference = `#${String(jobId || '').slice(0, 8).toUpperCase()}`;
    const totalLabel = formatCurrency(quote?.total_amount || breakdown.total || 0);
    const clientLabel = String(quote?.client_name || '').trim();
    const intro = clientLabel ? `Hola ${clientLabel},` : 'Hola,';
    const message = `${intro} te comparto tu presupuesto ${quoteReference} por ${totalLabel} para revisarlo online: ${shareLink}`;
    await shareOrCopyLink(shareLink, {
      title: `Presupuesto ${quoteReference}`,
      message,
      copiedLabel: 'Link del presupuesto listo para copiar o compartir',
    });
  };

  const handleShareFeedbackLink = async () => {
    if (isPreparingFeedbackLink) return;

    try {
      setIsPreparingFeedbackLink(true);
      ...
      showPlatformAlert('No pudimos generar el link', shareErr?.message || 'Intenta nuevamente.');
    } finally {
      setIsPreparingFeedbackLink(false);
    }
  };
  ...
  const primaryAction = isDraft
    ...
    : isClosedJob
      ? {
          label: isPreparingFeedbackLink ? 'Generando link...' : 'Pedir calificacion',
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={['bottom']} style={styles.footerWrap}>
        <View style={styles.footerBar}>
          ...
          <TouchableOpacity
            style={[styles.footerPrimaryBtn, isPreparingFeedbackLink && styles.footerPrimaryBtnDisabled]}
            onPress={primaryAction.onPress}
            disabled={isPreparingFeedbackLink}
          >
            {isPreparingFeedbackLink ? (
