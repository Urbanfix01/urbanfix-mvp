...
type RequestFilterKey = 'active' | 'decision' | 'closed' | 'all';
...
const isClosedRequest = (status: string) => status === 'completed' || status === 'cancelled';

const getSubmittedResponsesCount = (request: ClientRequest) =>
  (request.responses || request.quotes || []).filter((response) =>
    isSubmittedResponse(String(response.quoteStatus || ''))
  ).length;

const hasPendingDecision = (request: ClientRequest) =>
  !isClosedRequest(request.status) &&
  !Boolean(request.assignedTechId || request.selectedQuoteId) &&
  getSubmittedResponsesCount(request) > 0;
...
export default function ClientRequestsScreen() {
  ...
  const [selectedFilter, setSelectedFilter] = useState<RequestFilterKey>('active');
    ...
    () => [
      {
        key: 'active' as const,
        label: 'Activas',
        count: requests.filter((request) => !isClosedRequest(request.status) && !hasPendingDecision(request)).length,
        description: 'Seguimiento abierto y trabajos en curso.',
      },
      {
        key: 'decision' as const,
        label: 'Por decidir',
        count: requests.filter((request) => hasPendingDecision(request)).length,
        description: 'Ya tienes respuestas para revisar y elegir.',
      },
      {
        key: 'closed' as const,
        label: 'Cerradas',
        count: requests.filter((request) => isClosedRequest(request.status)).length,
        description: 'Finalizadas o canceladas con historial completo.',
      },
      {
        key: 'all' as const,
        label: 'Todas',
        count: requests.length,
        description: 'Vista general de toda tu operacion cliente.',
      },
    ],
    [requests]
  );

  const selectedFilterMeta =
    requestFilterOptions.find((item) => item.key === selectedFilter) || requestFilterOptions[0];

  const visibleRequests = useMemo(() => {
    const filtered = requests.filter((request) => {
      if (selectedFilter === 'decision') return hasPendingDecision(request);
      if (selectedFilter === 'closed') return isClosedRequest(request.status);
      if (selectedFilter === 'active') return !isClosedRequest(request.status) && !hasPendingDecision(request);
      return true;
    });

    return filtered.sort((left, right) => {
      const bucketLeft = hasPendingDecision(left) ? 0 : isClosedRequest(left.status) ? 2 : 1;
      const bucketRight = hasPendingDecision(right) ? 0 : isClosedRequest(right.status) ? 2 : 1;
      if (bucketLeft !== bucketRight) return bucketLeft - bucketRight;
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        >
          ...
          >
            {requestFilterOptions.map((option) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.filterChipLabel, selected && styles.filterChipLabelActive]}>{option.label}</Text>
                  <View style={[styles.filterChipCount, selected && styles.filterChipCountActive]}>
                    <Text style={[styles.filterChipCountText, selected && styles.filterChipCountTextActive]}>
                      {option.count}
                    </Text>
                  </View>
                </TouchableOpacity>
          ...
          <View style={styles.filterIntroCard}>
            <View style={styles.filterIntroIcon}>
              <Ionicons
                name={
            ...
            <View style={styles.filterIntroCopy}>
              <Text style={styles.filterIntroTitle}>{selectedFilterMeta.label}</Text>
              <Text style={styles.filterIntroText}>{selectedFilterMeta.description}</Text>
            </View>
          </View>
          ...
          {requests.length > 0 && visibleRequests.length === 0 && (
            <View style={styles.emptyFilteredCard}>
              <Text style={styles.emptyFilteredTitle}>Sin resultados en {selectedFilterMeta.label.toLowerCase()}</Text>
              <Text style={styles.emptyFilteredText}>
                Cambia el filtro o publica una nueva solicitud para seguir moviendo tu operacion.
              </Text>
            </View>
          )}
          ...
          {visibleRequests.map((request) => {
            ...
            const requestPriorityTone = hasPendingDecision(request)
              ? styles.requestCardDecision
              : isClosedRequest(request.status)
                ? styles.requestCardClosed
                : null;

            return (
              <View key={request.id} style={[styles.requestCard, requestPriorityTone]}>
                ...
                <View style={styles.requestInsightRow}>
                  <View style={styles.requestInsightCard}>
                    <Text style={styles.requestInsightLabel}>Revision</Text>
                    <Text style={styles.requestInsightValue}>
                      {hasPendingDecision(request) ? 'Pendiente' : requestHasChosenTechnician ? 'Definida' : 'Abierta'}
                    </Text>
                  </View>
                  <View style={styles.requestInsightCard}>
                    <Text style={styles.requestInsightLabel}>Tecnico</Text>
                    <Text style={styles.requestInsightValue} numberOfLines={1}>
                      {request.assignedTechName || request.targetTechName || 'Sin elegir'}
                    </Text>
                  </View>
                  <View style={styles.requestInsightCard}>
                    <Text style={styles.requestInsightLabel}>Respuestas</Text>
                    <Text style={styles.requestInsightValue}>{responseCount}</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  filterChipLabel: { fontFamily: FONTS.subtitle, color: '#475569', fontSize: 12 },
  filterChipLabelActive: { color: '#9A3412' },
  filterChipCount: {
    minWidth: 24,
    paddingHorizontal: 6,
    paddingVertical: 3,
  ...
  filterChipCountText: { fontFamily: FONTS.subtitle, color: '#64748B', fontSize: 10 },
  filterChipCountTextActive: { color: '#9A3412' },
  filterIntroCard: {
    flexDirection: 'row',
  ...
  filterIntroCopy: { flex: 1, gap: 2 },
  filterIntroTitle: { fontFamily: FONTS.subtitle, color: '#9A3412', fontSize: 13 },
  filterIntroText: { fontFamily: FONTS.body, color: '#9A3412', fontSize: 12, lineHeight: 18 },
  ...
  emptyFilteredTitle: { fontFamily: FONTS.subtitle, color: '#0F172A', fontSize: 15 },
  emptyFilteredText: { fontFamily: FONTS.body, color: '#64748B', fontSize: 13, lineHeight: 20 },
  requestsStack: { gap: 14 },
  requestCard: {
    backgroundColor: COLORS.white,
    borderRadius: 18,
    borderWidth: 1,
  ...
  requestInsightRow: { flexDirection: 'row', gap: 8, flexWrap: 'wrap' },
  requestInsightCard: {
    flexGrow: 1,
    flexBasis: '30%',
    minWidth: 88,
  ...
  requestInsightValue: { fontFamily: FONTS.subtitle, color: '#0F172A', fontSize: 12 },
  assignedCard: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
