...
export default function ClientProfileScreen({
  ...
  const loadProfile = useCallback(
    ...
    [loadNearbyTechnicians, requiredCompletion]
  );
  ...
  const availableCount = nearbyTechnicians.filter((technician) => technician.available_now).length;
  const selectedTechnician =
    nearbyTechnicians.find((technician) => technician.id === selectedTechnicianId) || nearbyTechnicians[0] || null;
  const profileHighlights = useMemo(
    () => [
      ...
      },
      {
        key: 'network',
        label: 'Red',
        value: requiredCompletion ? '--' : String(nearbyTechnicians.length),
      },
      {
        key: 'active',
        label: 'Activos',
        value: requiredCompletion ? '--' : String(availableCount),
      },
    ],
    [availableCount, city, nearbyTechnicians.length, requiredCompletion]
  );
  ...
  const MenuOption = ({ icon, label, onPress, isNew = false }: any) => (
        ...
        >
          {requiredCompletion ? (
            <View style={styles.requiredCard}>
              ...
              <Text style={styles.requiredTitle}>Para entrar a la plataforma necesitamos tus datos personales.</Text>
              <Text style={styles.requiredText}>
                Completa nombre, telefono y ciudad antes de continuar.
              </Text>
            </View>
          ) : (
            <>
              <View style={styles.brandCard}>
                <View style={styles.brandBanner}>
                  ...
                  <View style={styles.brandBannerContent}>
                    ...
                    <Text style={styles.brandBannerTitle}>Tu base para coordinar servicios con mas contexto</Text>
                  </View>
                </View>
              ...
              <View style={styles.card}>
                <View style={styles.sectionHeader}>
                  <View style={styles.sectionCopy}>
                    <Text style={styles.sectionEyebrow}>Red cercana</Text>
                    <Text style={styles.cardTitle}>Tecnicos cerca de tu ubicacion</Text>
                  </View>
                  <View style={styles.sectionPill}>
                    <Text style={styles.sectionPillText}>
                      {nearbyTechnicians.length ? `${nearbyTechnicians.length} activos` : 'Sin datos'}
                    </Text>
                  </View>
                </View>
                ...
                {!!selectedTechnician && (
                  <View style={styles.contextCard}>
                    ...
                    <Text style={styles.contextText}>
                      Tecnico destacado: {selectedTechnician.name} | {selectedTechnician.distance_km.toFixed(1)} km
                    </Text>
                  </View>
                )}

                {!!nearbyPayload?.warning && (
                  <View style={styles.noticeCard}>
                    <Ionicons name="alert-circle-outline" size={16} color="#92400E" />
                    <Text style={styles.warningText}>{nearbyPayload.warning}</Text>
                  </View>
                )}
                {!!nearbyError && (
                  <View style={[styles.noticeCard, styles.noticeCardError]}>
                    <Ionicons name="warning-outline" size={16} color="#991B1B" />
                    <Text style={styles.errorInline}>{nearbyError}</Text>
                  </View>
                )}
                        ...
                        return (
                          ...
                          >
                            <View style={styles.nearbyRowMain}>
                              <View style={styles.nearbyRowTitleWrap}>
                                <Text style={styles.nearbyName}>{technician.name}</Text>
                                <View
                                  style={[
                                    styles.availabilityBadge,
                                    technician.available_now ? styles.availabilityBadgeOn : styles.availabilityBadgeOff,
                                  ]}
                                >
                                  <Text
                                    style={[
                                      styles.availabilityBadgeText,
                                      technician.available_now
                              ...
                              <Text style={styles.nearbyAddress} numberOfLines={1}>
                                {[technician.address, technician.city].filter(Boolean).join(', ') || 'Sin direccion visible'}
                              </Text>
                            </View>
                            <View style={styles.nearbyRowRight}>
                              <Text style={styles.nearbyDistance}>{technician.distance_km.toFixed(1)} km</Text>
                              <Text style={styles.nearbyHint}>{selected ? 'Seleccionado' : 'Toca para enfocar'}</Text>
                            </View>
                          </TouchableOpacity>
                        );
                      })}
